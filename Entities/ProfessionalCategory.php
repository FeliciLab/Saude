...
class ProfessionalCategory extends \MapasCulturais\Entity {
	...
	public static function getCategoryProfessional($idCategory) {
		$app = App::i();
		$dql = "SELECT p.id, p.name FROM \Saude\Entities\ProfessionalCategory p where p.id = " . $idCategory;
		$query = $app->em->createQuery($dql);
		$all = $query->getResult();
		return $all;
	}
	...
	public static function createAgentMeta($agent, $key, $value) {
		$app = App::i();
		$dql = "INSERT INTO  MapasCulturais\Entities\AgentMeta a 
		(a.key, a.value, a.owner) VALUES ({$key},{$value}, {$agent})";
		$query      = $app->em->createQuery($dql);
