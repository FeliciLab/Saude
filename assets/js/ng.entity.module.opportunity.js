...
module.controller('EvaluationMethodConfigurationController', ['$scope', '$rootScope', 'RelatedAgentsService', 'EvaluationMethodConfigurationService', 'EditBox', 'OpportunityApiService', 'evaluationCandidate', function($scope, $rootScope, RelatedAgentsService, EvaluationMethodConfigurationService, EditBox, OpportunityApiService, evaluationCandidate) {
        ...
        $scope.createAdminRelation = function(entity){
            ...
            //PARA QUEM ESTÁ INSCRITO NA OPORTUNIDADE E É SELECIONADO PARA AVALIADOR
            var messageEvaluation = false;
            // VERIFICA SE O AVALIADOR JÁ ESTÁ INSCRITO NA OPORTUNIDADE
                evaluationCandidate.verifyCandidate(entity.id, getOpportunityId()).then(function successCallback(response) {
                    if(response.data.message == false) {
                        isAgentRelation = true;
                        // HABILITANDO PARA SER GERADO A MENSAGEM
                        messageEvaluation = true;
                    }else{
                        // RETORNA TRUE SEO MESMO JÁ ESTIVER COMO AVALIADOR NESSA OPORTUNIDADE
                    ...
                    if (!isAgentRelation) {
                        ...
                        _scope.spinnerCondition = false;
                        _scope.noEntityFound  = false;
                        if(messageEvaluation == true){
                            MapasCulturais.Messages.error('Esse Agente já está inscrito nessa oportunidade, portanto não poderá ser um avaliador!');
                        }else{
...
module.controller('OpportunityController', ['$scope', '$rootScope', '$location', '$anchorScroll', '$timeout', 'RegistrationService', 'EditBox', 'RelatedAgentsService', '$http', 'UrlService', 'OpportunityApiService', '$window', function ($scope, $rootScope, $location, $anchorScroll, $timeout, RegistrationService, EditBox, RelatedAgentsService, $http, UrlService, OpportunityApiService, $window) {
    ...
    $scope.setStatusToSelected = function () {
        new PNotify({
            title: 'Um minuto!',
            text: 'Já estamos processando a alteração...',
            type: 'info',
        ...
        RegistrationService.setStatusCandidate();
    }
...
module.factory('evaluationCandidate',['$http', function($http){
    // var url = new UrlService('taxonmias');
        return {
            // CONSULTA NO HOOKS DO THEME.PHP
            verifyCandidate: function(idAgent, opportunity) {
                return $http.get(MapasCulturais.baseURL +'opportunity/evaluationCandidate/?opportunity='+opportunity+'&idAgent='+idAgent)
            }
        }
}]);

module.controller('evaluationCandidateController', ['$scope' , '$http', 'evaluationCandidate', function ($scope , $http, evaluationCandidate) {
    console.log('evaluationCandidateController')
