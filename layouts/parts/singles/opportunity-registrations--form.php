...
if ($entity->isRegistrationOpen()): ?>
    <?php if ($app->auth->isUserAuthenticated()): ?>
        ...
        <form id="opportunity-registration" class="registration-form clearfix">
            <p class="registration-help" style="margin-left: -70px;"><?php \MapasCulturais\i::_e("Para iniciar sua inscrição, selecione o agente responsável. Ele deve ser um agente individual (pessoa física), com um CPF válido preenchido.");?></p>
            <div>
                <div id="select-registration-owner-button" class="input-text margin-left-70" ng-click="editbox.open('editbox-select-registration-owner', $event)">{{data.registration.owner ? data.registration.owner.name : data.registration.owner_default_label}}</div>
            ...
            <div>
                <a class="btn btn-primary" style="color: #ffffff;" ng-click="register()" rel='noopener noreferrer'><?php \MapasCulturais\i::_e("Fazer inscrição");?></a>
            </div>
        </form>
        <?php endif; ?>
    <?php else: ?>
