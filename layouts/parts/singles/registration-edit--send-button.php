<div class="registration-fieldset">
    ...
    <div>
        ...
        <p class="registration-help">
            <?php \MapasCulturais\i::_e('Essa inscrição tem um DAE para pagamento'); ?>
        </p>
        <p>
            <a href="<?php echo $app->createUrl('dae', 'gerarBoleto'); ?>" target="_blank" class="btn btn-accent">
                <i class="fa fa-file-text"></i> Gerar DAE
            </a>
        </p>
        <hr>
    </div>
    ...
    <?php if(!$entity->opportunity->isRegistrationOpen() && $entity->canUser('send')): ?>
        ...
        <?php else: ?>
            <a ng-click="sendRegistration()" class="btn btn-danger hltip" data-hltip-classes="hltip-danger" hltitle="<?php \MapasCulturais\i::esc_attr_e('Somente super admins podem usar este botão e somente deve ser usado para enviar inscrições que não foram enviadas por problema do sistema.'); ?>" data-status="<?php echo MapasCulturais\Entities\Registration::STATUS_SENT ?>">
                <i class="fa fa-paper-plane"></i>
                <?php \MapasCulturais\i::_e("enviar esta inscrição");?></a>
        <?php endif; ?>
    <?php endif ?>
</div>
